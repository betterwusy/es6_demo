...
const cart = {
  ...
  get wheels () {	
  // 等同于 get wheels: function() { }
    return this._wheels;
  },

  set wheels (value) {
  // 等同于 set wheels: function() {}
...
let o2 = { b: 2 };
o2.__proto__ = o1;
let { ...o3 } = o2; // o3是用扩展运算符进行解构赋值的，所以 o3 不能复制 o2 继承自原型的属性
o3 // { b: 2 }
o3.a // undefined
```
...
let { x, ...newObj } = o;   
// x 是单纯的解构赋值，所以x可以读到o继承的属性，即 x = 3;
// ...newObj 是扩展运算符的解构赋值，所以 newObj = { z: 3 }
let { y, z } = newObj;
x // 1
y // undefined
z // 3
...
变量`x`是单纯的解构赋值，所以可以读取对象`o`继承的属性；而 ...newObj 是扩展运算符的解构赋值，所以无法读取对象o继承的属性，即 newObj = { z: 3 }

变量`y`和`z`是扩展运算符的解构赋值，只能读取对象`o`自身的属性，所以变量`z`可以赋值成功，变量`y`取不到值。

另外：ES6 规定，变量声明语句之中，如果**使用解构赋值，扩展运算符后面必须是一个变量名，而不能是一个解构赋值表达式**，所以上面代码引入了中间变量`newObj`，如果写成下面这样会报错。
