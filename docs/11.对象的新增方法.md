...
上面代码中，`Object.assign()`把数组视为属性名为 0、1、2 的对象，因此源数组的 0 号属性 `4` 覆盖了目标数组的 0 号属性 `1` 、1号属性 `5` 覆盖了目标数组的1号属性 `2` 。
...
ES5 的`Object.getOwnPropertyDescriptor()`方法会返回某个对象属性的描述对象（descriptor）。ES2017 引入了`Object.getOwnPropertyDescriptors()`方法，返回**指定对象所有自身属性（非继承属性）的描述对象**。
