...
### 1）for...of 遍历 Generator 函数生成的遍历器对象

`for...of` 循环可以自动遍历 Generator 函数运行时生成的遍历器对象，且此时不再需要调用 `next` 方法。
...
### 3）使用 Generator 函数生成的遍历器对象部署到对象的[Symbol.iterator]属性上
...
**如果 Generator 函数内部没有部署`try...catch`代码块，那么遍历器对象`throw` 方法抛出的错误，将被外部`try...catch`代码块捕获。**
...
**如果 Generator 函数内部有 `try...finally` 代码块，且正在执行 `try` 代码块，那么 `return()` 方法会导致立刻进入 `finally` 代码块。并且执行完改代码块以后，整个函数才会结束。**
